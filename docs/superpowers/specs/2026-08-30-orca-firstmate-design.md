...
2. **Run**: for each sub-task -> the `brief` skill generates a spec -> `task-create --spec` -> ⚠ **CORRECTED 2026-09-02, see `docs/verification/2026-09-02-smoke-real-loop.md` §1: `--worktree new-top-level` has never worked (`selector_not_found`, measured twice, the second time with `--name` and a valid `--repo` both present). Creating a worktree is its own command: `orca worktree create --name <n> --repo <sel> --setup run`, THEN `worker-start --worktree path:<the new path>` with no creation flags. The line below is kept as the record of what was planned, not as instructions.** -> `worker-start --task <id> --worktree new-top-level --name <n> --agent claude [--model ... --effort ...] [--on <host>] --setup run`. Every task in the request -- including review fixes, retries, spawned work -- **inherits the chosen host, no re-asking**. Delivery mode is the opposite: locked in per task at creation time (see the Delivery mode section), written into the request file with a one-line reason whenever it departs from the project's posture.
